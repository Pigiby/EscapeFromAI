...
synthesized audio is returned as in-memory WAV bytes.

The voice model is loaded once per process via `functools.cache`.
...
def get_tts(voice_path: str = DEFAULT_VOICE_PATH, length_scale: float = DEFAULT_LENGTH_SCALE) -> PiperTTS:
    ...
    if not resolved.exists():
        raise FileNotFoundError(
            ...
            f"Download the voice into voice_negotiation/assets/voices/."
        )
